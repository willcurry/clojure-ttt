(ns clojure-ttt.computer
  ...
  (defn- score [board depth marks]
    (cond (and (any-wins? board) (= (last-move board) (first marks))) depth
  ...
  (defn- initial-score [marks player]
    (if (= (first marks) player) {:score -1000 :board nil} {:score 1000 :board nil}))

  (defn- favours-computer? [marks player new-score current-best]
    (cond 
    (= (first marks) player) (> new-score (:score current-best)) 
    :else (< new-score (:score current-best))))
